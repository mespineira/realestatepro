...
function rep_sideload_featured($post_id, $url, $desc = '')
{
    ...
    $tmp = wp_tempnam($url);
    if (!$tmp)
        return new WP_Error('temp_error', 'No se pudo crear archivo temporal');
    ...
    $response = wp_safe_remote_get($url, array(
        ...
        'filename' => $tmp,
        'headers' => array(
            'User-Agent' => 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) RealEstatePro/1.0'
        )
    ));
    ...
    if (wp_remote_retrieve_response_code($response) !== 200) {
        @unlink($tmp);
        return new WP_Error('http_error', 'HTTP ' . wp_remote_retrieve_response_code($response));
    }

    // Explicit clear stat cache so filesize checks correct size.
