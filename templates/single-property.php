...
<main class="rep-container single-property">
  <?php while (have_posts()):
    ...
    $anyo = get_post_meta($pid, 'anyo_construccion', true);
    ...
    <article <?php post_class('rep-single rep-card-xl'); ?>>
      ...
      <header class="rep-head">
        ...
        <div class="rep-head-right">
          <?php if ($precio): ?>
            <div class="rep-price-lg"><?php echo esc_html(rep_price_format($precio)); ?></div><?php endif; ?>
        </div>
      </header>
      ...
      <!-- Bloque de datos rápidos (Quick Facts) -->
      <div class="rep-quick-facts">
        <?php if($m2): ?>
        <div class="rep-qf-item" title="Superficie">
          <i class="fa-solid fa-ruler-combined"></i>
          <span><?php echo esc_html($m2); ?> m²</span>
        </div>
        <?php endif; ?>
        
        <?php if(get_post_meta($pid, 'terraza', true)): ?>
        <div class="rep-qf-item" title="Terraza">
          <i class="fa-solid fa-umbrella-beach"></i>
          <span>1</span>
        </div>
        <?php elseif(get_post_meta($pid, 'balcon', true)): ?>
        <div class="rep-qf-item" title="Balcón">
          <i class="fa-solid fa-umbrella-beach"></i>
          <span>1</span>
        </div>
        <?php endif; ?>
        ...
        <?php if($hab): ?>
        <div class="rep-qf-item" title="Habitaciones">
          <i class="fa-solid fa-bed"></i>
          <span><?php echo esc_html($hab); ?></span>
        </div>
        <?php endif; ?>
        
        <?php if($ban): ?>
        <div class="rep-qf-item" title="Baños">
          <i class="fa-solid fa-bath"></i>
          <span><?php echo esc_html($ban); ?></span>
        </div>
        <?php endif; ?>
        
        <?php if(get_post_meta($pid, 'garaje', true)): ?>
        <div class="rep-qf-item" title="Garaje">
          <i class="fa-solid fa-car"></i>
          <span>1</span>
        </div>
        <?php endif; ?>

        <?php if(get_post_meta($pid, 'anyo_construccion', true)): ?>
        <div class="rep-qf-item" title="Año de construcción">
          <i class="fa-solid fa-building"></i>
          <span><?php echo esc_html(get_post_meta($pid, 'anyo_construccion', true)); ?></span>
        </div>
        <?php elseif(get_post_meta($pid, 'ascensor', true)): ?>
        <div class="rep-qf-item" title="Edificio con ascensor">
          <i class="fa-solid fa-building"></i>
          <span>Sí</span>
        </div>
        <?php endif; ?>
      ...
      <section id="fotos" class="rep-section">
        <div class="rep-gallery" data-rep-gallery>
          <div class="rep-gallery-main">
            ...
            <div class="rep-g-stage">
              ...
              <?php else: ?>
                <img class="rep-g-current" src="<?php echo esc_url(rep_placeholder_img()); ?>"
            ...
            <div class="rep-gallery-thumbs">
              ...
              <div class="rep-gt-strip">
                <?php foreach ($slides as $idx => $s): ?>
                  <img class="rep-gt-thumb <?php echo $idx === 0 ? 'is-active' : ''; ?>" src="<?php echo esc_url($s['thumb']); ?>"
                    data-med="<?php echo esc_url($s['med']); ?>" data-full="<?php echo esc_url($s['full']); ?>"
        ...
        <div class="rep-lightbox" data-rep-lightbox hidden>
          ...
          <div class="rep-lb-thumbs">
            <?php foreach ($slides as $idx => $s): ?>
              <img class="rep-lb-thumb <?php echo $idx === 0 ? 'is-active' : ''; ?>" src="<?php echo esc_url($s['thumb']); ?>"
                data-full="<?php echo esc_url($s['full']); ?>" alt="Miniatura lightbox <?php echo esc_attr($idx + 1); ?>" />
            <?php endforeach; ?>
          </div>
        </div>
      </section>
      ...
      <?php if ($lat !== '' && $lng !== ''): ?>
        <section id="mapa" class="rep-section">
          <h2>Ubicación</h2>
          <div class="rep-map" data-rep-map data-lat="<?php echo esc_attr($lat); ?>"
            data-lng="<?php echo esc_attr($lng); ?>" data-title="<?php echo esc_attr(get_the_title()); ?>"></div>
        ...
        <div class="rep-two">
          ...
          <aside class="rep-box">
            ...
            <ul class="rep-features">
              <?php if ($m2): ?>
                <li><strong>Superficie:</strong> <?php echo esc_html($m2); ?> m²</li><?php endif; ?>
              <?php if ($hab): ?>
                <li><strong>Habitaciones:</strong> <?php echo esc_html($hab); ?></li><?php endif; ?>
              <?php if ($ban): ?>
                <li><strong>Baños:</strong> <?php echo esc_html($ban); ?></li><?php endif; ?>
              <?php if ($ref): ?>
                <li><strong>Referencia:</strong> <?php echo esc_html($ref); ?></li><?php endif; ?>
              <?php if ($anyo): ?>
                <li class="rep-feature-highlight"><strong>Construcción:</strong> Año <?php echo esc_html($anyo); ?></li>
              <?php endif; ?>
              <?php 
              $anyo_reforma = get_post_meta($pid, 'anyo_reforma', true);
              if ($anyo_reforma): ?>
                <li class="rep-feature-highlight" style="background:#e8f0fe; color:#1a73e8; border-color:#d2e3fc;"><strong>Reforma:</strong> Año <?php echo esc_html($anyo_reforma); ?></li>
              <?php endif; ?>
              <?php 
              $estado_cons = get_post_meta($pid, 'estado_conservacion', true);
              if ($estado_cons): ?>
                <li class="rep-feature-highlight" style="background:#fce8e8; color:#d93025; border-color:#fad2cf;"><strong>Estado:</strong> <?php echo esc_html(ucfirst(strtolower($estado_cons))); ?></li>
              <?php endif; ?>
              <?php
              foreach ($features as $k) {
                if (isset($all_feature_labels[$k]))
                  echo '<li>' . esc_html($all_feature_labels[$k]) . '</li>';
              } ?>
            </ul>
                  ...
                  <div class="rep-energy-rows">
                    ...
                    foreach ($letters as $L => $data):
                      ...
                      <div class="rep-energy-row <?php echo ($rating === $L) ? 'is-active' : ''; ?>">
                        <div class="rep-energy-letter"
                          style="background-color:<?php echo $data['color']; ?>; color: <?php echo in_array($L, ['A', 'B', 'C', 'F', 'G']) ? '#fff' : '#000'; ?>;">
                          <?php echo $data['label']; ?>
                        </div>
                        <div class="rep-energy-value">
                          <?php if ($rating === $L && !$is_tramite && !empty($e_cons))
                            echo '<span>' . esc_html(number_format_i18n($e_cons, 2)) . '</span>'; ?>
                        </div>
                        <div class="rep-energy-value">
                          <?php if ($rating === $L && !$is_tramite && !empty($e_emis))
                            echo '<span>' . esc_html(number_format_i18n($e_emis, 2)) . '</span>'; ?>
                        </div>
                      </div>
                    <?php endforeach; ?>
                  </div>
